...
func newPlaylistsCreateCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Short: "Create a new playlist (cost: 50 units)",
		Long: "Create a new playlist on the authenticated user's account.\n\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRun {
				fmt.Fprintf(cmd.OutOrStderr(),
					"DRY RUN: would create playlist title=%q privacy=%s description=%q (cost: 50 units)\n",
